...
udp_init(void) {
    ...
    if(net_event_subscribe(event_handler, NULL) == -1) {
        errorf("net_event_subscribe() filure");
        return -1;
    }
    return 0;
}
...
udp_bind(int id, struct ip_endpoint *local)
{
    ...
    if(exist) {
        debugf("the endpoint is already used, input: %s, found: %s",
...
udp_recvfrom(int id, uint8_t *buf, size_t size, struct ip_endpoint *foreign)
{
    ...
    while(1) {
        ...
        /* Wait to be woken up by sched_wakeup() or sched_intterupt() */
