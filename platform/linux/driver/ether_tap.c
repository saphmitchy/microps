...
ether_tap_open(struct net_device *dev)
{
    ...
    tap = PRIV(dev);
    tap->fd = open64(CLONE_DEVICE, O_RDWR);
    if(tap->fd == -1) {
        errorf("open: %s, dev=5s", strerror(errno), dev->name);
        return -1;
    }
...
ether_tap_init(const char *name, const char *addr)
{
    ...
    infof("ethernet devie initialized, dev=%s", dev->name);
    return dev;
}
